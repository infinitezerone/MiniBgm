...
> ✅ 该约束已于 2026-09-22 在全仓库（生产代码 / AI 提示词 / 界面文案 / 各模块测试）核对过：生产代码残留 0。
>
...
5. **站点硬编码退场** —— ✅ **首轮已完成（2026-09-22）**：唯一的内置站点定制嗅探已删除（连同 AI 提示词与
   测试里的站点知识，生产代码残留 0）。这一项**没有等 1–4 全部具备就动手**，理由是先用新增用例做了
   **等价性验证**：`FETCH 分页 → EXTRACT_VARIABLE 抽参 → POST 取 cookie（captureHeaders）→ EXTRACT_STREAM`
   全链跑通，确认用户可导入的规则能表达同等能力后才删代码。被规避的正是"用户能力先回落"这个风险本身，
   不是把顺序颠倒。余下四项（列表语义 / 断言门 / 嗅探判定表 / 规则健康度）仍未做。

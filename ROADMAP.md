...
> **做最懂二次元、交互最流畅的现代化 Bangumi 客户端**  
> 本路线图围绕二次元核心体验三大支柱——**追番 (Tracking)**、**找番 (Discovery)**、**社区 (Community)** 以及 **系统级现代智能化 (System Intelligence)** 制定，旨在系统性解决现有功能断层，打造标杆级移动端体验。
...
```
                             ...
                             │
MiniBgm 核心能力矩阵 ────────┼─── 2. 找番 (Discovery) ─── 多维交叉筛选与知识图谱
                             │
                             ├─── 3. 社区 (Community) ─── 原生单集吐槽与讨论交流
                             │
                             └─── 4. 智能 (Intelligence) ─ Generative UI 与系统级意图外露
```
...
## 4. 现代系统级智能 (System & Generative AI)
...
```mermaid
flowchart TD
    M1["v0.3.0 · 追番链路闭环 & 单集吐槽\n(批量打卡 + Deep Link 唤醒 + 原生单集吐槽)"] --> M2["v0.4.0 · 原生社区 & 季度导视\n(原生讨论帖详情 + BBCode 回帖 + 季度新番导视网格)"]
    M2 --> M3["v0.5.0 · 深度探索 & 知识图谱\n(声优/Staff 详情页 + 复合多维筛选 + Top 250 榜单)"]
    M3 --> M4["v0.6.0 · 社交时间线 & 系统级 AI\n(全站/好友动态流 + Generative UI + AppFunctions 外露)"]
```

| 里程碑版本 | 核心主题 | 关键交付内容 |
| :--- | :--- | :--- |
| **v0.3.0** | **追番闭环 & 单集吐槽** | 1. 单集长按“看到此集”批量打卡<br>2. 播放源 Deep Link 原生唤醒官方客户端<br>3. 原生单集独立吐槽楼与剧透遮罩 |
| **v0.4.0** | **原生讨论区 & 季度导视** | 1. 原生帖子详情页与楼中楼回复（彻底告别跳浏览器）<br>2. 季度新番导视总览大盘<br>3. 开播日历系统写入与提醒 |
| **v0.5.0** | **高维探索 & 知识图谱** | 1. 监督/声优/制作人员图谱详情页<br>2. 高级多维复合过滤器<br>3. Top 250 与冷门佳作官方榜单 |
| **v0.6.0** | **动态时间线 & 现代系统级 AI** | 1. 好友与全站看番动态时间线 (Timeline)<br>2. AI 助手 Generative UI 原生卡片返回<br>3. 接入 Android AppFunctions 供系统级语音调用 |

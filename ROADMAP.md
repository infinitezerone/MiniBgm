...
- [x] **AI 找源的边界：只转述工具结果，交付可播放数据** ⭐ *本期确立并已落地*
  ...
  - 模型本身**不生成、不拼接、不改写任何地址**——地址只能来自 `findPlayableSources` 的返回，取数顺序固定为「用户自备片单 → 条目记录的来源页 → 解析这些页面」。
  - 抓取受硬约束：只按给定 URL 单页取一次，最多 5 页、正文 512 KB 上限（`PageFetchService`）；不做站点遍历、不批量、不缓存结果，一律由用户在会话中显式触发。

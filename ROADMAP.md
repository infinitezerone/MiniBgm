...
### 阶段目标与功能规划
- [x] **AI 找源的能力边界** ⭐ *本期确立并已落地*
  - AI 助手可以替用户检索并返回**可观看的页面链接**，交由用户自行打开——这与搜索引擎同性质。
  - **不**输出绕过来源站访问校验后取得的媒体直链；**不**自动或批量执行（一律用户显式触发）；**不**缓存检索结果。
  - 原"AI 智能嗅探播放源"入口已按此边界收敛：条目页只把提问交接给助手会话（`AssistantRoute(prefillPrompt)`），检索由 `:core:ai` 的找源工具返回来源站番剧页 / Bangumi 条目与分集页 / 站点搜索页；智能体调用被架构红线限制在 `:feature:assistant` 内。
...
- **AI 直接取流** — 受第 5 节"AI 找源的能力边界"约束，只到页面链接为止。
- **全屏播放手势** — 押后（理由见第 1 节）。

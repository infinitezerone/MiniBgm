...
9. **AI lives in the assistant conversation only**: `:core:ai` may be depended on by `:app` and `:feature:assistant` — no other feature may touch it, and no page may call an agent directly. A screen that wants an AI answer navigates to `AssistantRoute(prefillPrompt = …)`, which the assistant consumes exactly once. Source finding splits two permissions, and the split is the point: playable addresses come **only** from `findPlayableSources` (imported playlists → the user's `kind = SOURCE` rules → the source pages recorded for the entry), and its JSON is what renders as an episode list and reaches `PlayerRoute` together with the request headers. The site-reversing SOP tools (`probeSiteAndFindSample`, `traceNetworkTraffic`, `inspectPageStructure`, `testPlaybackRule`) are the only place a model-supplied URL gets fetched, and their output may only ever be a `PENDING_CONFIRMATION` proposal — nothing reaches `SettingsRepository.playbackRules` until the user approves the card. Fetch caps: `PageFetchService` takes one URL per call, truncates at 512 KB and drops CR/LF/NUL headers; `resolvePages` stays under 5 pages and 12 candidates each — a rule's own `pipeline` step list is currently the only un-budgeted fan-out. A rule's `kind`/`parserType`/`pipeline` triple must have an execution path (`PlaybackSourceRule.isResolvable`); both the AI proposal and the manual batch import reject the rest instead of silently downgrading to sniffing. When static sniffing descends from a list/search page into an episode page it must re-verify the page still belongs to the requested title (`pageBelongsToTitle`): a search that finds nothing typically falls back to an unrelated "latest anime" list, whose `[25]`-style anchors match the episode-number rule, so an unverified descent reports success while pointing at a different show. Enforced by `ArchitectureRulesTest.only_assistant_feature_may_invoke_the_ai_agent`, `BgmAgentPromptTest` (playback grounding + confirmation gate), `ToolRegistryIntegrationTest`, `PlaybackSourceRuleTest` and `EpisodeListDescentGuardTest`.
...
4. **Safe ADB screenshots**: never use bare `adb exec-out screencap -p > file.png` (emulator multi-display warnings corrupt PNG magic headers and break multimodal API calls). Always specify `-d 0` (`adb exec-out screencap -d 0 -p > ...`) or capture on-device first (`adb shell screencap -p /data/local/tmp/s.png && adb pull ...`), and verify with `file <file>.png` before passing to `view_file`. Under Git Bash export `MSYS_NO_PATHCONV=1` first — MSYS rewrites `/sdcard/x.png` into a Windows path, so the device silently writes to a bogus location and every later `pull` fails. A ~20 KB all-black PNG plus a clock in `uiautomator dump` means the screen fell asleep: send `KEYCODE_WAKEUP` and re-capture rather than concluding the UI is broken.
5. **动态门禁已废除（2026-09）**：`scripts/dual-screen-verify.sh` 不再是任何改动的必跑项，不要主动提议或执行它。布局/导航/骨架类改动的行为验证改为开发者自行决定是否做模拟器走查；导航栈不变量仍由 `BgmNavStatePropertyTest` 静态覆盖。

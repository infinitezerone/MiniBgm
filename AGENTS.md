...
9. **AI lives in the assistant conversation only**: `:core:ai` may be depended on by `:app` and `:feature:assistant` — no other feature may touch it, and no page may call an agent directly. A screen that wants an AI answer navigates to `AssistantRoute(prefillPrompt = …)`, which the assistant consumes exactly once. Per the ROADMAP §5 source-finding boundary, AI returns watchable **page links** only (site entry/episode pages, Bangumi pages, search pages) — never a resolved media direct URL, never automatic or batched retrieval. Enforced by `ArchitectureRulesTest.only_assistant_feature_may_invoke_the_ai_agent`; the page-link contract is asserted on the tool description in `ToolRegistryIntegrationTest`.

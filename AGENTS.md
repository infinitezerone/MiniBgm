...
9. **AI lives in the assistant conversation only**: `:core:ai` may be depended on by `:app` and `:feature:assistant` — no other feature may touch it, and no page may call an agent directly. A screen that wants an AI answer navigates to `AssistantRoute(prefillPrompt = …)`, which the assistant consumes exactly once. Playable addresses come **only** from `findPlayableSources` (the model never generates playback URLs); tool outputs proposing new rules must be confirmed by the user before saving into `SettingsRepository.playbackRules`. Enforced by `ArchitectureRulesTest.only_assistant_feature_may_invoke_the_ai_agent`, `BgmAgentPromptTest` (playback grounding + confirmation gate), `PlaybackSourceRuleTest` and `EpisodeListDescentGuardTest`.
...
- **Module boundaries are asserted at configuration time**: the root project applies `minibgm.module.boundary` (`ModuleBoundaryConventionPlugin` in build-logic), which after `projectsEvaluated` walks every configuration's declared `ProjectDependency` and fails the build on `:feature:* → :feature:*`, `:feature:* → :core:network/:core:database/:core:datastore`, or non-assistant `:feature:* → :core:ai` — so any `./gradlew` invocation enforces redlines 1/2/9 on the real dependency graph, immune to dynamically-built `project(":…")` strings. Path access follows Now in Android's `Graph.kt` (read `ProjectDependency.path` only, never touch the target `Project` — Isolated-Projects-safe). Catalog aliases for `minibgm.*` plugins deliberately declare **no version** (NIA's pattern): `version = "unspecified"` breaks resolution once a `minibgm.*` plugin is applied at root ("already on the classpath with an unknown version"). `ArchitectureRulesTest` keeps only source-semantic checks (imports, colors, raw IO, `MutableStateFlow`, credentials, route sealing) — its build-script-text scans were removed as redundant, but import scans stay as tripwires for future `api` leakage from `:core:data` (the compiler can't see a slow leak, only each instant).
- **Module READMEs carry a generated dependency graph**: the same root plugin registers `graphDump` / `graphUpdate` on every module (ported from NIA's `Graph.kt`, Apache-2.0). Run `./gradlew graphUpdate` after adding/removing module edges; it writes the Mermaid graph between `<!--region graph-->` markers in each module's `README.md`, appending the section to hand-written READMEs rather than clobbering them. KMP edges live in `*MainApi`/`*MainImplementation` configurations — the default configuration whitelist covers them; `graph.supportedConfigurations` / `graph.ignoredProjects` properties override. Not CI-enforced: READMEs can go stale, the config-phase assertion is what's binding.
...
./gradlew graphUpdate                         # regenerate per-module README dependency graphs (after module edges change)
...
4. **Safe ADB screenshots**: never use bare `adb exec-out screencap -p > file.png` (emulator multi-display warnings corrupt PNG magic headers and break multimodal API calls). Always specify `-d 0` (`adb exec-out screencap -d 0 -p > ...`) or capture on-device first (`adb shell screencap -p /data/local/tmp/s.png && adb pull ...`), and verify with `file <file>.png` before passing to `view_file`. On Windows Git Bash, export `MSYS_NO_PATHCONV=1` to prevent path rewriting.
...
- **纯装饰特性必须 fail-open**：光晕/动效/横幅等不影响核心信息获取的增强，任何内部异常都应降级为"没有该装饰"（`runCatching` + 默认值），绝不允许让宿主页面崩溃。读硬件位图像素前先 `copy` 成软件位图。
- Reuse `BgmHttpClient.jsonConfig` (`ignoreUnknownKeys`, `isLenient`, `coerceInputValues`, `encodeDefaults`, no pretty-print) instead of hand-rolling `Json` instances. Enforced by `ArchitectureRulesTest.json_instances_have_a_single_home_per_scope`: main sources may construct `Json { … }` only in a whitelisted home per scope — `BgmHttpClient.jsonConfig` (default), `core/ai` `aiJson` (LLM protocol needs `explicitNulls = false`), `UserPreferencesSerializer` (persistence), `feature/user` `PlaybackRulesViewModel` (features cannot see jsonConfig). New scope → new whitelisted home with a stated reason; never a second inline copy.
...
- **Detached HEAD is normal**: `git HEAD` is permanently detached onto jj's working copy `@`; never report it as an issue or run `git switch`. Cite revisions by jj change id (`git` hashes change on rewrite).
- **Avoid amending pushed commits**: `@` is a commit; edit in a new revision after pushing (`jj git push && jj new`).

...
9. **AI lives in the assistant conversation only**: `:core:ai` may be depended on by `:app` and `:feature:assistant` — no other feature may touch it, and no page may call an agent directly. A screen that wants an AI answer navigates to `AssistantRoute(prefillPrompt = …)`, which the assistant consumes exactly once. Per the ROADMAP §5 source-finding boundary, the agent **never produces a URL of its own**: playable results come only from `findPlayableSources`, which reads the user's imported playlists, then the user-configured third-party source APIs (`PlaybackSourceRule` with `kind = SOURCE`, template + optional fixed request headers), then the source pages recorded for the entry, resolving those pages under hard caps (one fetch per configured URL, <=5 pages, 512 KB body, CR/LF/NUL header injection rejected — `PageFetchService`). Its JSON is rendered as a playable episode list and handed to `PlayerRoute` together with the request headers; retrieval is always user-triggered, never batched or cached. Enforced by `ArchitectureRulesTest.only_assistant_feature_may_invoke_the_ai_agent`; the tool-grounding contract is asserted on the tool description in `ToolRegistryIntegrationTest` and on the system prompt in `BgmAgentPromptTest`.

...
internal val BGM_AGENT_SYSTEM_PROMPT: String =
    ...
    - 当用户想“寻找/搜索播放源并导入”且未提供网址时：目标是检索开源社区 TVBox/聚合订阅源。限定在 GitHub 检索（调用 searchWeb，关键词如“site:github.com tvbox 动漫 源”或“site:github.com tvbox 接口”）；若检索到相关仓库，可调用 fetchWebContent 查看 README 提取 .json 订阅直链，再调用 validateAndTestSubscription 测试校验并生成待确认提案。

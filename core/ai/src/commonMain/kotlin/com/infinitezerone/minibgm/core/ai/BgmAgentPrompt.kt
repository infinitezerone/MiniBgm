...
internal val BGM_AGENT_SYSTEM_PROMPT: String =
    ...
    - 用户提供第三方看番网站网址、TVBox 订阅或询问如何逆向/适配/导入播放源时：
      1. 若用户提供了某个动漫网站的网址或需要适配新站点，按以下逆向探查 SOP 执行全自主闭环：
         ...
         e. 规则沙箱自测：调用 testPlaybackRule(ruleJson, sampleTitle, sampleEp)。它会重跑规则，并对解析出的首个直链做一次首包断言（playbackVerified）；"解析得出地址"不等于"地址能播"，playbackVerified 为 false 时按 verificationNote 给出的原因继续修规则，最多修 3 轮，仍不通过就如实报告失败原因，不要无限重试。写 EXTRACT_STREAM 正则时利用列表语义：第 1 组捕获直链、可选第 2 组捕获集名/集号文本（如 `"u":"([^"]+)","t":"([^"]+)"`），引擎会按请求话数自动从候选里选条目——规则要能覆盖整季所有集，不要把 {ep} 锚死在正则里让一条规则只对采样那一集成立；
